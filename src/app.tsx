...
type EntryItemProps = { height: string; header: string; text: string }
const entries: EntryItemProps[] = [
  {
    height: '400vh',
    header: 'headset',
...
function EntryItem({ height, header, text }: EntryItemProps) {
  const styles = css({
    pointerEvents: 'none',
    position: 'sticky',
    top: 0,
...
const Overlay = forwardRef<HTMLDivElement, OverlayProps>(({ caption, scroll }, ref) => {
  ...
  return (
    <div ref={ref} onScroll={scrollHandler} className={styles}>
      {entries.map((entry, ind) => <EntryItem key={ind} {...entry} />)}
